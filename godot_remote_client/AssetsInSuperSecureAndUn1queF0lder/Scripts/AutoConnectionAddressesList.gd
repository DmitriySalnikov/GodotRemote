...
func _on_auto_connection_list_changed(new_list):
	...
	for dict in new_list:
		...
		tmp_item.setup_params(version, ip, project_name, port, addresses, img)
...
func _update_rect_size():
	...
	var sum_height = h.rect_size.y + style.content_margin_bottom + style.content_margin_top
	if sum_height < 238:
		rect_size.y = sum_height
	else:
		rect_size.y = 238
